...
const cardData = [
  {
    id: 1,
    icon: <BsCamera2 size={70} className="text-white" />,  
    title: 'Photography',
    description:
  ...
  {
    id: 2,
    icon: <BsFilm size={70} className="text-white" />,  
    title: 'Filmmaking',
    description:
  ...
  {
    id: 3,
    icon: <GiSaxophone size={70} className="text-white" />,  
    title: 'Music & Jazz',
    description:
    ...
    <div className="min-h-screen  flex items-center justify-center flex-wrap ">
      ...
      {cardData.map((card) => (
        <Link key={card.id} href={card.contactLink}>
          ...
          <div id="card" className="relative m-4 cursor-pointer">
            ...
            <div
              id="content"
              className="p-8 w-72 bg-gray-950/50 border-2 border-gray-100/50 rounded-xl shadow-lg relative space-y-7"
            >
              <div className="bg-orange-600 h-3 w-16 ml-auto"></div>
              
              <div className="text-center mb-4">{card.icon}</div>
              <h2 className="text-2xl font-semibold text-orange-500">{card.title}</h2>
